...
function  reload (e) {
  if (document.URL.split('?')[1]) {
    window.location.href = window.location.href.split('?')[0]+ e;
    window.location .href= window.location.href.split('?')[0]+ e;
...
function LinkRenderer(props) {
  if (props.children[0].split("{")[1] == 'newtab}') {
    props.children = props.children[0].split("{")[0];
    props.href = document.URL.split('?')[0]+ props.href;
    // return <a onclick="window.open ( props.href, ''); return false" href="javascript:void(0);"></a>
  return <button class="markdowna" href={props.href} target="_blank"  onClick={preventReload}>{props.children}</button>
  }  else {
    return <button class="markdowna" onClick={() => reload(props.href)}>{props.children}</button>
  }
